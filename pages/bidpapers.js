...
function BidPapers(props) {
    ...
    async function bidForPaper(id) {
        try {
            let result = await axios.post('/api/bidForPaper', {paperID: id, reviewerEmail: props.email, preferredNumber: numberOfPapers});
...
export default function bidPapers() {
    ...
    if(status == "authenticated") {
        if(session.user.userCategory == "Reviewers") {
            return <BidPapers email={session.user.username}></BidPapers>
        }
